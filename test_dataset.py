...
import torch
import numpy as np
class pet_test_dataset(Dataset):
    def __init__(self, test_dir):
        ...
        for i in range(0, len(all_patient_slice_list), 5):
            ...
            self.list_test.append(group)
        self.root_singram = os.path.join(test_dir, '1_sino/')
        self.root_gt_sin = os.path.join(test_dir, 'gt_sino/')
        self.root_img = os.path.join(test_dir, 'gt_img/')
    ...
    def __getitem__(self, index):
        ...
        slice_group = self.list_test[index]
        ...
        return pet_singram_volume, pet_img_volume, pet_gt_singram_volume, name
